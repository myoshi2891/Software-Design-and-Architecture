...
const GROUPS: NavGroup[] = [
  ...
  {
    title: "詳細設計",
    items: [{ id: "s3", num: "03", label: "基本設計原則" }],
  },
];
...
describe("MonolithicArchitectureSidebar", () => {
  ...
  beforeEach(() => {
    ...
    document.body.insertAdjacentHTML(
      "beforeend",
      `<main>
        <section class="doc-section" id="s1"></section>
        <section class="doc-section" id="s2"></section>
        <section class="doc-section" id="s3"></section>
  ...
  it("グループ見出しと nav リンクをソース順で描画する", () => {
    const { container } = render(<MonolithicArchitectureSidebar groups={GROUPS} />);
    const groupTitles = Array.from(container.querySelectorAll(".nav-grp")).map(
      (el) => el.textContent
    );
    expect(groupTitles).toEqual(["基礎知識", "詳細設計"]);
  ...
  it("クラスを持たないプレーンな section は IntersectionObserver に登録されない", () => {
    render(<MonolithicArchitectureSidebar groups={GROUPS} />);
