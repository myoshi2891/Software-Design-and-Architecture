...
describe("monolithic-architecture-comprehensive-guide page", () => {
  it("h1 見出しが完全リファレンスのタイトルである", () => {
    const { container } = render(<Page />);
    const h1 = container.querySelector("h1");
    expect(h1).not.toBeNull();
    expect(h1?.textContent).toContain("モノリシックアーキテクチャ");
  ...
  it("外部リンクすべてに target=_blank と rel=noopener noreferrer が付く", () => {
    ...
    expect(external.length).toBe(49);
  ...
  it("Mermaid 図 16 個・table 7 個・コードブロック 9 個を描画する", () => {
    const { container } = render(<Page />);
    expect(container.querySelectorAll(".mermaid")).toHaveLength(16);
  ...
  it("globals.css に .monolithic-architecture-comprehensive-guide .main のレイアウト調整が含まれている", () => {
    ...
    const scopeClass = ".monolithic-architecture-comprehensive-guide";
