...
describe("hexagonal-architecture-comprehensive-guide page", () => {
  it("h1 見出しが完全リファレンスのタイトルである", () => {
    const { container } = render(<Page />);
    const h1 = container.querySelector("h1");
    expect(h1).not.toBeNull();
    expect(h1?.textContent).toContain("ヘキサゴナルアーキテクチャ");
    expect(h1?.textContent).toContain("完全ガイド");
  });

  it("h2 セクション見出しが 15 個ある", () => {
    const { container } = render(<Page />);
  ...
  it("Mermaid 図 14 個・table 6 個・コードブロック 11 個を描画する", () => {
    const { container } = render(<Page />);
  ...
  it("globals.css に .hexagonal-architecture-comprehensive-guide .main のレイアウト調整が含まれている", () => {
    ...
    const sectionContent = cssContent.slice(
      cssContent.indexOf(".hexagonal-architecture-comprehensive-guide")
    );
