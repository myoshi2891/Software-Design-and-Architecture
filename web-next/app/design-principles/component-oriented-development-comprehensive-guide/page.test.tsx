...
describe("component-oriented-development-comprehensive-guide page", () => {
  it("h1 見出しが完全リファレンスのタイトルである", () => {
    const { container } = render(<Page />);
    const h1 = container.querySelector("h1");
    expect(h1).not.toBeNull();
    expect(h1?.textContent).toContain("コンポーネント指向開発");
  ...
  it("Mermaid 図 21 個・table 7 個・コードブロック 14 個を描画する", () => {
    const { container } = render(<Page />);
    expect(container.querySelectorAll(".mermaid")).toHaveLength(21);
  ...
  it("globals.css に .component-oriented-development-comprehensive-guide .main のレイアウト調整が含まれている", () => {
    const fs = require("node:fs");
    const path = require("node:path");
    const cssPath = path.resolve(__dirname, "../../globals.css");
    const cssContent = fs.readFileSync(cssPath, "utf-8");

    const scopeClass = ".component-oriented-development-comprehensive-guide";
