...
export const CHECKLIST_ITEMS: readonly ChecklistItem[] = [
  ...
  { id: "chk-2", text: "マルチエージェント化によって得られる価値が、約15倍というトークンコスト増に見合うかを見積もったか" },
  { id: "chk-3", text: "各エージェントの分離境界（何を知らせ、何を知らせないか）を明示的に設計したか" },
  { id: "chk-4", text: "「書き込み」を行うエージェントを単一（Single-Writer）に絞ったか" },
  { id: "chk-5", text: "各サブエージェントへの委任指示に「目的」「出力フォーマット」「使うツール」「タスクの境界」を明記したか" },
  { id: "chk-6", text: "コンテキストが肥大化した場合の圧縮・ノートテイキング戦略を用意したか" },
  { id: "chk-7", text: "エージェント間通信にMCP／A2Aなどの標準プロトコルを活用できないか検討したか" },
  { id: "chk-8", text: "長期記憶・共有メモリ・チェックポイントの保存先と保持期間を設計したか" },
  { id: "chk-9", text: "各ツールに最小権限を割り当て、高リスク操作には人間承認ゲートを設けたか" },
  { id: "chk-10", text: "Lethal Trifecta（プライベートデータ・信頼できない入力・外部通信）が同一セッションに同時に揃っていないかを監査したか" },
  { id: "chk-11", text: "OWASP Top 10 for Agentic Applicationsに照らしたリスク評価を行ったか" },
  { id: "chk-12", text: "OpenTelemetry GenAI Semantic Conventions等でトレース・評価結果を計装したか" },
  { id: "chk-13", text: "評価駆動開発のサイクル（失敗パターンの収集→評価基準の見直し）を回せる体制があるか" },
];
