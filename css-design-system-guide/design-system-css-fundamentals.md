...
詳細度は `(a, b, c, d)` の4桁の数値で表します（インラインスタイルがある場合は `(1,0,0,0)`）。

| セレクタの種類 | 詳細度 | 例 |
|---------------|--------|-----|
| インラインスタイル | (1,0,0,0) | `style="color:red"` |
| IDセレクタ | (0,1,0,0) | `#header` |
| クラス・属性・疑似クラス | (0,0,1,0) | `.button`, `[type]`, `:hover` |
| タグ・疑似要素 | (0,0,0,1) | `div`, `::before` |
| ユニバーサル | (0,0,0,0) | `*` |
...
#content        { color: green; }   /* (0,1,0,0) ← さらに勝つ */
p.text#content  { color: red; }     /* (0,1,1,1) ← 最も強い */
```
