...
- **ノード**: 人・会社・商品などの「モノ」。**ラベル**(種類)は0個以上持てる
...
リレーショナルDBでも、外部キーや中間テーブルを使えば関係そのものを明示的にモデル化できます。苦手になりやすいのは、その関係を**たどる**クエリのほうです。関係は行と行の対応として表現され、たどるたびにJOINで再構築するため、多段の関係(友達の友達の友達、など)をたどるクエリでは、結合の段数とデータ量に応じてコストが増えやすくなります。またスキーマ変更にマイグレーションコストがかかり、ドメインの進化に追従しにくいという課題もあります。いずれも一般的な傾向であり、実際のコストは結合アルゴリズム・インデックス・データ分布に左右されます。

### NoSQL集約指向DBの弱点

Key-Value・Document・Column-familyといった「集約指向(aggregate-oriented)」なNoSQLデータベースは、1つのまとまり(集約)の中のデータには強い一方、集約をまたぐ関係の扱いは相対的に手薄になりがちです。ただしこれも絶対的な制約ではなく、ドキュメント間の参照を持たせたり、製品が備えるクエリ機能(MongoDBの`$lookup`など)やマテリアライズドビューを使って集約間の関係を扱える場合もあります。

グラフDBとの違いは、関係を「集約に付随する情報」としてではなく、**独立した辺そのものとして第一級に扱う**点にあります。関係が複雑になるほど、集約指向側では表現とクエリ、そして運用のコストが増えていくのに対し、グラフDBはその複雑さを直接モデルへ載せられる、というトレードオフとして理解するのが実態に近いです。
...
flowchart TD
    ...
    G5 --> G6{"距離が判明した未確定ノードが残っているか"}
    G6 -->|"はい"| G2
    G6 -->|"いいえ"| G7["開始ノードから到達可能なノードの最短距離が確定"]
```
...
| アルゴリズム | 目的 | 重みを考慮した最短路への対応 | 代表的な用途 |
|---|---|---|---|
| BFS | 最短ホップ数を求める | 非対応(重み付きグラフを走査することはできるが、重みを考慮した最短路は保証しない) | SNSでの最短ホップ数、到達可能性判定 |
| DFS | 経路を網羅的にたどる | 非対応(重み付きグラフを走査することはできるが、重みを考慮した最短路は保証しない) | 循環検出、連結成分の抽出 |
| Dijkstra法 | 重み付きグラフの最短距離を求める | 対応(非負の重みのみ) | カーナビ、経路探索エンジン |
| A* | ヒューリスティック付きの最短距離を求める | 対応(非負の重みが前提。木探索ではヒューリスティックが admissible(真の残りコストを過大評価しない)であること、クローズ済みノードを再オープンしないグラフ探索では consistent(一貫性: h(n) ≤ c(n,n') + h(n'))であることが必要。より良い経路発見時にノードを再オープンするグラフ探索なら admissible で足りる) | ゲームAIの経路探索、地図アプリ |
...
2026年時点の一部の解説記事では、GQL標準化とAIツーリングの成熟によって、新規のグラフDBプロジェクトがGremlinよりもCypher(あるいはGQL)を選ぶ傾向にあること、GraphRAGが「不正検知・レコメンデーション以外で初めて主流化したグラフDBのユースケース」だと位置づけられています。いずれも個別記事の見立てであり、市場全体の統計として裏づけられたものではない点に注意してください。
...
| GQL | Cypherのクエリ構成のセマンティクスを多く取り込み、2024年にISO/IEC 39075として国際標準化されたグラフクエリ言語 |
...
4. Neo4j — GraphAcademy「Neo4j Fundamentals」コース(グラフの基礎概念を扱う無料コース)
