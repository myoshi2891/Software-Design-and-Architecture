...
|---|---|---|---|
| BFS | 最短ホップ数を求める | 非対応(重み付きグラフを走査することはできるが、重みを考慮した最短路は保証しない) | SNSでの最短経路(次数)、到達可能性判定 |
| DFS | 経路を網羅的にたどる | 非対応(重み付きグラフを走査することはできるが、重みを考慮した最短路は保証しない) | 循環検出、連結成分の抽出 |
| Dijkstra法 | 重み付きグラフの最短距離を求める | 対応(非負の重みのみ) | カーナビ、経路探索エンジン |
| A* | ヒューリスティック付きの最短距離を求める | 対応(非負の重みに加え、ヒューリスティックが真の残りコストを過大評価しない admissible 条件を満たす場合) | ゲームAIの経路探索、地図アプリ |
