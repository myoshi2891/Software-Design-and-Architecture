...
- **Spanner**（Google）: **TrueTime**（GPS と原子時計に支えられた分散時刻 API で、現在時刻を一点ではなく「不確実性区間」として返す）を活用するアプローチ。TrueTime だけで外部一貫性が得られるわけではなく、コミット時にその不確実性区間が過ぎ去るまで待つ **commit-wait** を組み合わせることで、「先にコミットしたトランザクションのタイムスタンプは必ず小さい」ことを保証し、グローバル分散環境での外部一貫性を実現する
