...
| メモリ型（In-Memory / Memory-Based） | 主要な処理をメモリ上で完結させ、高速なアクセスを実現する。ただしメモリは揮発性であるため、耐久性（Durability）の確保が別途必要になる | Redis、SAP HANA（の一部モード） |
...
データベースが突然クラッシュしても、コミット済みのトランザクションのデータが失われてはいけません。これを実現する中心的な仕組みが **WAL（Write-Ahead Log）** です。「変更されたデータページをディスクへ書き出す前に、必ずその変更内容を記録したログを安定記憶へ永続化しておく」という原則です。メモリ上のページは先に書き換えて構わず、制約がかかるのはあくまで**ディスクへの書き出し順序**である点が要点で、これによりランダムなページ書き込みを遅延させつつ、シーケンシャルなログ書き込みだけを同期化できます。この順序に加えて、あるトランザクションにコミット成功を返す前に、その COMMIT レコードまでのログを安定記憶へ永続化しておく必要があります（ログフォース）。この2つが守られていれば、クラッシュ後の再起動時にログから正しい状態を復元できます。ただし復元は「ログを再生するだけ」では済みません。未コミットの更新を含む dirty page もディスクへ書き出してよい Steal 構成では、クラッシュ時点のディスク上に未コミットの更新が混ざりうるためです。ARIES に代表される手順では、まずログ上の更新を Redo してクラッシュ時点の状態を再現し、そのうえで未コミットだった更新を Undo することで、整合性のとれた状態へ戻します。
...
flowchart LR
    ...
    Analysis --> Redo["② Redoフェーズ：Analysisが求めたRedoLSNから走査し、ページLSN等の状態を確認して未反映の更新だけを再適用（未コミット分も対象）"]
...
flowchart TB
    ...
    MVCC["マルチバージョン（MVCC）：バージョン管理の手法"] --> Snapshot["書き込みと並行して、読み取りは過去のスナップショットを参照する"]
    MVCC -. 組み合わせ可能 .-> Pess
    MVCC -. 組み合わせ可能 .-> Opt
```
...
- **マルチバージョン同時実行制御（MVCC）**: 各行について複数のバージョンを保持し、読み取りは書き込みをブロックせずに自分から見えるバージョンを参照する。どの範囲でスナップショットが固定されるか（文単位か、トランザクション単位か）は分離レベルと実装に依存し、読み取りトランザクション全体が常に同一のスナップショットを見るとは限らない。PostgreSQLをはじめ多くの現代的なデータベースが採用。MVCCは悲観的・楽観的と並ぶ択一の方式ではなく、バージョンの持ち方に関する手法であり、書き込み同士の競合はロック（悲観的）やコミット時検証（楽観的）と組み合わせて制御する
...
B-Tree が「その場更新」を基本とするのに対し、**LSM-Tree（Log-Structured Merge-Tree）** は書き込みを主に「追記」として扱い、後段のコンパクションでデータを再編成する、まったく異なる設計思想のストレージ構造です。Cassandra、RocksDB、LevelDB、HBase など、多くの現代的な NoSQL データベースがこの方式を採用しています。

LSM-Tree の基本的な考え方は次の通りです。

1. 書き込みは、まずメモリ上のソート済み構造（**Memtable**、多くは Skiplist などで実装）へ即座に反映される。実装や設定で WAL が有効な場合は、同時に耐障害性のために WAL へも書き込まれる。WAL は無効化できる設定を持つ製品もあり（RocksDB の `disableWAL`、Cassandra の `durable_writes = false` など）、無効にするとフラッシュ前の書き込みはクラッシュ時に失われる。また、WAL を有効にしただけで永続性が保証されるわけではなく、WAL をいつディスクへ同期・フラッシュするかの設定（RocksDB の `WriteOptions::sync`、Cassandra の `commitlog_sync` など）にも依存する。非同期書き込みの設定では、マシン障害（OS クラッシュや電源断）の際に直近の更新が失われうる
...
flowchart TB
    ...
    Write --> WALog["WALへの書き込み（WAL有効時のみ。永続性の保証度は同期・フラッシュ設定に依存）"]
...
原著では、この読み取り・書き込み・空間の3つの増幅（**RUM予想**：Read／Update／Memory amplification）というトレードオフの枠組みで、LSM-Tree の設計判断を評価しています。さらに関連する設計として、LSM-Tree とは別系統の、追記専用ログとメモリ上のハッシュテーブル（keydir）を組み合わせたログ構造化ハッシュテーブルである Bitcask（Riak の初期ストレージエンジン）や、LSM-Tree を土台にキーとバリューを分離して格納し（キーのみLSM-Treeに、バリューは別ログに置く）コンパクションコストを削減する派生設計の WiscKey も紹介されています。
...
| 観点 | B-Tree系（主にその場更新型。Copy-on-Write B-Tree は除く） | LSM-Tree系（ログ構造化型） |
|---|---|---|
...
- **Next-In-Line Failover**: Bullyアルゴリズムの最適化。リーダーが代替ノード（フェイルオーバー先）のリストを事前に共有しておき、リーダー故障時には検知したノードがそのリストの最上位の生存ノードに直接連絡することで、再選出にかかる手順を短縮する
- **Candidate/Ordinary Optimization**: ノードを「候補ノード（candidate）」と「通常ノード（ordinary）」の2集合に分け、リーダーになれるのは候補ノードのみとする。通常ノードが選挙を開始する際は候補ノード群にだけ問い合わせ、応答した中で最上位のノードをリーダーとすることで、選挙時のメッセージ数を減らす
...
- **結果整合性（Eventual Consistency）**: 新しい書き込みが止まったうえで、レプリカ間の通信と修復（アンチエントロピーなど）が継続し、各更新が最終的にすべての到達可能なレプリカへ配送・反映されるという前提のもとで、いずれそれらのレプリカが同じ値に収束することだけを保証する、最も緩いモデル。書き込みが止まるだけでは収束しない

さらに、Cassandra や DynamoDB に見られる「チューナブル（調整可能）な一貫性」、Spanner や Apache Cassandra などに見られる「Witness Replica」、そして特定のデータ型と演算規則（可換・冪等な併合など）を満たす範囲で、ネットワーク分断中に並行して行われた更新を調整なしに収束させられる **CRDT（Conflict-free Replicated Data Type）** と、条件を満たす CRDT が「配信済みの更新をすべて受け取ったレプリカは同じ状態に収束する」ことを保証する性質である **強収束性（Strong Eventual Consistency）** といった、実践的で発展的な話題まで扱われています。
...
- **Hinted Handoff**: 書き込み先のレプリカが一時的に応答不能な場合、別のノードが代わりにその書き込みを「ヒント」として一時保存しておき、対象ノードの復帰後に再生（転送）を試みる。ただしヒントの保持期限切れや保持ノード自身の障害で再生されないこともあるベストエフォートの仕組みであり、レプリカ間の完全な収束にはアンチエントロピー修復（下記の Merkle Tree による差分同期など）が別途必要になる
