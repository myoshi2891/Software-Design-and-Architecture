...
| 出版社 | O'Reilly Media |
| 初版発行 | 2019年10月 |
| ページ数 | 約376ページ（紙版）／ xix + 351ページ（電子版の物理的記述） |
...
ここで重要なのは、SQL のパースやクエリ最適化を行う「クエリプロセッサ」層と、実際にバイト列をディスクへ読み書きする「ストレージエンジン」層は、はっきり分離されているという点です。実際、SQLite・MySQL（InnoDB や MyRocks）・MongoDB（WiredTiger）のように、上位のクエリ層を保ったまま、下位のストレージエンジンだけを差し替えられる設計のデータベースも存在します。本書はこの下側、つまりストレージエンジンと分散データ管理の部分に的を絞った一冊です。
...
| 列指向（Column-Oriented） | 同じカラムの値だけをまとめて連続して保存する（ワイドカラムストアもこの発展形として位置づけられる） | OLAP（分析処理）：特定の数カラムだけを大量の行にわたって集計する処理が多い場合 |

例えば「全顧客の平均年齢を求める」というクエリでは、`age` カラムの値だけを連続して読み込める列指向の方が、無駄なディスクI/Oが少なく高速になります。一方、「顧客IDで1件のレコード全体を取得する」というクエリでは、行指向の方が1回のディスクアクセスでレコード全体を読み込めるため有利です。本書では、Cassandra や Bigtable に代表される「ワイドカラムストア」も、この行指向・列指向の分類の延長線上にある特殊なケースとして整理されています。
...
二分探索木は1ノードに1つのキーしか持てず、木の高さ（＝ディスクアクセス回数）がキー数に対して対数的に増えていきます。これに対し B-Tree は、1ノード（多くの場合1ディスクページに対応）に多数のキーを持たせることで「扇（ファンアウト）」を大きくし、木の高さを低く保ちます。結果として、数百万〜数十億件のレコードがあっても、わずか3〜4回程度のディスクアクセスで目的のレコードにたどり着けます。
...
データベースが突然クラッシュしても、コミット済みのトランザクションのデータが失われてはいけません。これを実現する中心的な仕組みが **WAL（Write-Ahead Log）** です。「データページ本体を更新する前に、必ずその変更内容をログへ先に書き込んでおく」というシンプルな原則ですが、これによりクラッシュ後の再起動時に、ログを再生するだけで正しい状態を復元できます。
...
```mermaid
flowchart LR
    Crash["クラッシュ発生・再起動"] --> Analysis["① Analysisフェーズ：ログを読み、クラッシュ時点の状態を復元"]
    Analysis --> Redo["② Redoフェーズ：コミット済みの変更をすべて再適用"]
...
- **マルチバージョン同時実行制御（MVCC）**: 各行の複数バージョンを保持し、読み取りトランザクションは書き込みをブロックせずに一貫したスナップショットを参照できる。PostgreSQLをはじめ多くの現代的なデータベースが採用
...
| Lazy B-Tree / Lazy-Adaptive Tree | 変更をすぐには下位ノードへ反映せず、いったんノードにバッファリングしてまとめて適用する。書き込みをバッチ化することでI/Oを削減する | WiredTiger の一部設計 |
...
1. 書き込みは、まずメモリ上のソート済み構造（**Memtable**、多くは Skiplist などで実装）へ即座に反映される。同時に、耐障害性のために WAL にも書き込まれる
...
4. バックグラウンドで定期的に「**コンパクション**」が実行され、複数の SSTable がマージされて、より少数の・より大きな SSTable へと統合される。この過程で、削除済み（tombstone）のデータや、上書きされて不要になった古いバージョンのデータが実際に取り除かれる
...
- **FLP不可能性**: 非同期分散システムにおいて、たとえ1つのプロセスが故障するだけでも、決定的なアルゴリズムでは合意（コンセンサス）を保証できないことを示した、Fischer・Lynch・Paterson による有名な定理
...
分散システムでは、あるノードが「本当にクラッシュしたのか」「単にネットワークが遅いだけなのか」を、確実に区別する方法は理論上存在しません（FLP不可能性）。そのため実践的なシステムでは、「疑わしさ」を確率的・ヒューリスティックに判定する障害検出器（Failure Detector）が使われます。
...
最もシンプルな方式は、一定間隔で「ハートビート」を送り合い、決められたタイムアウトを超えたら故障と「みなす」というものです。しかし、固定のタイムアウト値は「ネットワークが混雑している普段の状況」と「本当に落ちた状況」を区別できず、誤検出を生みがちです。この問題への改良として **Φ (Phi) Accrual障害検出器** が紹介されています。これは固定の閾値で二値判定するのではなく、過去のハートビート到着間隔の統計分布から「このノードが生きている確率」を連続値（疑わしさスコア）として算出する方式で、Cassandra や Akka などで実際に採用されています。また、全ノードが個別に監視するのではなく、ゴシッププロトコル（STEP 16）を通じて障害情報自体を伝播させる方式も紹介されています。
...
分散システムの議論で頻繁に登場する **CAP定理** は、「Consistency（一貫性）」「Availability（可用性）」「Partition tolerance（ネットワーク分断耐性）」の3つのうち、ネットワーク分断が実際に発生した際には、CとAの両方を同時に完全には満たせない、ということを示しています。原著では、CAP定理を単純化しすぎず、その正しい適用範囲（分断が実際に起きている「間」の話であって、平常時の話ではない）を丁寧に解説しています。
...
```mermaid
flowchart LR
    A["線形化可能性（Linearizability）"] -- 保証は弱くなるが性能・可用性は上がる --> B["逐次一貫性（Sequential Consistency）"]
    B -- 保証は弱くなるが性能・可用性は上がる --> C["因果一貫性（Causal Consistency）"]
    C -- 保証は弱くなるが性能・可用性は上がる --> D["結果整合性（Eventual Consistency）"]
```
...
さらに、Cassandra や DynamoDB に見られる「チューナブル（調整可能）な一貫性」、Riak などに見られる「Witness Replica」、そしてネットワーク分断中でも書き込みの競合を数学的に自動解消できる **CRDT（Strong Eventual Consistency and Conflict-free Replicated Data Types）** といった、実践的で発展的な話題まで扱われています。
...
- **Spanner**（Google）: 高精度な時刻同期の仕組み（TrueTime）を活用し、グローバル分散環境でも外部一貫性を実現するアプローチ
...
Part II ― そして本書全体 ― の集大成にあたるのが、第14章で扱われる **コンセンサス（合意形成）** です。「クラスタ内の複数ノードが、ある1つの値について、故障が発生しても全員が同じ結論に到達できることを保証する」問題であり、分散システム理論の中でも特に難易度が高い領域とされています。
...
flowchart LR
    ...
    Replicate -- 過半数から確認応答を受信 --> Commit["エントリをコミットしてステートマシンへ適用"]
```
...
| 付録 | A | Bibliography | 参考論文・書籍100冊以上、10冊以上の書籍を含む参考文献一覧 |
