...
さらに、Cassandra や DynamoDB に見られる「チューナブル（調整可能）な一貫性」、Riak などに見られる「Witness Replica」、そして特定のデータ型と演算規則（可換・冪等な併合など）を満たす範囲で、ネットワーク分断中に並行して行われた更新を調整なしに収束させられる **CRDT（Conflict-free Replicated Data Type）** と、条件を満たす CRDT が「配信済みの更新をすべて受け取ったレプリカは同じ状態に収束する」ことを保証する性質である **強収束性（Strong Eventual Consistency）** といった、実践的で発展的な話題まで扱われています。
