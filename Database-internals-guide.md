...
二分探索木は1ノードに1つのキーしか持てず、木の高さ（＝ディスクアクセス回数）は、平衡が保たれている場合でもキー数に対して対数的に増えていきます。さらに平衡化されていない二分探索木では、挿入順によって木が一方向に伸び、高さがキー数に対して線形にまで悪化します。これに対し B-Tree は、1ノード（多くの場合1ディスクページに対応）に多数のキーを持たせることで「扇（ファンアウト）」を大きくし、木の高さを低く保ちます。結果として、数百万〜数十億件のレコードがあっても、わずか3〜4回程度のディスクアクセスで目的のレコードにたどり着けます。
...
Part II ― そして本書全体 ― の集大成にあたるのが、第14章で扱われる **コンセンサス（合意形成）** です。「クラスタ内の複数ノードが、ある1つの値について、一部に故障が発生しても、正常に稼働しているノード同士は同じ結論に到達できることを保証する」問題であり、分散システム理論の中でも特に難易度が高い領域とされています。
