...
本書の第4版が第3版（2012年）と大きく異なる点は、単なる内部構造の解説にとどまらず、**「サービスレベル目標（SLO）を起点に、チームとしてどう信頼性を作るか」** という運用思想を前面に出していることです。これは、著者の一人であるSilvia BotrosがSendGridで、数十億通のメール送信を支えるデータベースプラットフォームの構築を支援してきた経験に基づいています。
...
| MVCC | 通常の `SELECT`（一貫性読み取り）がロックを取らずにスナップショットを読める仕組み。READ COMMITTED では一貫性読み取りのたびに新しいスナップショットを取得し、REPEATABLE READ では同一トランザクション内の最初の一貫性読み取りで確立したスナップショットを使い続ける。どちらもこれが既定の動作で、読み取りと書き込みが互いをブロックしにくくなる。ロッキングリードは対象外で、最新行に対してロックを取る |
...
- **ディスクI/Oスケジューラ**: Linuxのマルチキュー環境では `none`（スケジューリングを行わずデバイスへ直接発行）と `mq-deadline`（各リクエストに期限を割り当て、待ち時間が過度に伸びないよう抑える。I/O 完了レイテンシの上限を保証するものではない）が主な選択肢。どちらが有利かはカーネルバージョン・デバイス・ワークロードで変わるため、実際のワークロードでベンチマークを取って決める
...
本書では、スキーマ変更（マイグレーション）を「属人的な手作業」から「データストア基盤の一部として自動化・レビュー可能なプロセス」に変えていくことの重要性が説かれています。オンラインDDLツール（gh-ost、pt-online-schema-changeなど）を使い、本番の読み書きを継続したままダウンタイムを最小化してスキーマ変更を行う運用が現在では一般的です。ただし完全な無停止ではなく、テーブルを切り替える cut-over 時には短時間のメタデータロックが発生します。適用前・適用中は、cut-over 時のメタデータロック待ち、レプリカ遅延、行コピーやトリガー・binlog 処理による追加負荷を監視する必要があります。
...
InnoDBのインデックスはB-Tree構造が基本です。行データそのものを格納する「クラスタードインデックス」と、それ以外の「セカンダリインデックス」の違いを理解することが出発点になります。

| インデックス種別 | 特徴 |
|---|---|
| クラスタードインデックス | 行データそのものを葉に格納するインデックス。InnoDBでは、主キーがあれば主キー、なければ NOT NULL な適切なUNIQUE索引、それもなければ内部生成のローIDが用いられる |
| セカンダリインデックス | 指定した列の値と、対応するクラスタードインデックスのキー値（通常は主キー値）を持つ。検索後にそのクラスタードインデックスのキーでデータ本体を引き直す |
...
- **冗長インデックス・重複インデックス**: `(a)` と `(a, b)` のように左端の列が共通するだけでは、冗長とは限らない。`(a)` が UNIQUE 制約として一意性を担っている場合や、列数の少ない `(a)` のほうが特定のクエリで有利な場合もある。削除する前に、一意性制約の有無と、実際のクエリでの使用状況（`sys.schema_redundant_indexes` / `sys.schema_unused_indexes` やスロークエリの `EXPLAIN` など）を確認して判断する
...
| テクニック | 内容 |
|---|---|
| 複雑なクエリの分割 | EXPLAINと実測でJOINがボトルネックだと確認できた場合に限り、1本の巨大なJOINクエリを複数のシンプルなクエリ＋アプリ側結合へ分けることを検討する。トランザクション境界・往復回数・N+1クエリを制御できることが前提で、常用すべき一般的な最適化手法ではない |
...
- **リードプール**: 複数のレプリカに読み取りクエリを分散する。非同期レプリケーションでは書き込み直後にレプリカを読むと反映前の古い値が返りうるため、自分の書き込みを読み返す必要がある読み取りはソースへ固定するか、対象トランザクションのレプリカ適用完了を待ってから読む（`WAIT_FOR_EXECUTED_GTID_SET` など）方針にする
- **ヘルスチェック**: 遅延しているレプリカやダウンしているレプリカをプールから自動的に外す。ただしヘルスチェックは閾値を超えた遅延を検出するだけで、閾値内の遅延による古い読み取りは防げない。半同期レプリケーションのACKもレプリカがバイナリログを受領した時点の保証であり、適用（反映）の完了までは保証しないため、これ単独でも解決にはならない
...
| 選択肢 | 特徴 |
|---|---|
| Amazon Aurora for MySQL | ストレージ層をAWSが独自最適化し、ストレージ容量が使用量に応じて自動拡張される点と高い可用性が特徴。コンピュート（インスタンス）はプロビジョンド構成では自動スケールせず、負荷に応じた自動増減が必要なら Aurora Serverless を選ぶ |
...
### MySQLのリリースモデルの変更（Innovation / LTS、2026年時点）

2023年7月18日、MySQLは新しいリリースモデルを発表しました。「Innovationリリース」（新機能を含む短いサポート期間のリリース）と「LTS（長期サポート）リリース」の2系統に分かれ、**最初のInnovationリリースは同日公開のMySQL 8.1.0**、**最初のLTSリリースは2024年公開のMySQL 8.4**です。本書執筆時点（2021年）にはまだ存在しなかった区分です。

さらに2026年7月28日公開の**MySQL 26.7.0**からは、Innovationリリースの版番号に年・月・パッチを表す**YY.M.P形式**（26.7.0 は2026年7月の初回リリース）が採用されました。
...
| innodb_buffer_pool_in_core_file | ON | OFF（コアダンプにInnoDBバッファプールのページを含めない） |
...
Perconaは2026年に入ってからも継続的にMySQLエコシステムのベンチマークを公開しています。2026年のPerconaのMySQLエコシステム性能検証レポートでは、MySQL（5.7 / 8.0 / 8.4 / 9.6）・Percona Server（5.7 / 8.0 / 8.4）・MariaDB（10.11 / 11.4 / 12.1）の計10バージョンがsysbench OLTPで比較されています（Percona Server の 9.x 系は比較対象に含まれていません）。バッファプールサイズ・並列度・ネットワーク越しの実行といった条件別に、バージョンごとの性能特性の違いが報告されています。本書の内容を実践に移す際は、こうした最新のベンチマークデータも合わせて確認することが推奨されます。
...
- **刊行が2021年11月であること**: Innovation / LTSリリースモデル（2023年発表、最初のLTSは2024年のMySQL 8.4）や、動的Redoログ（MySQL 8.0.30、2022年）など、本書執筆後に登場した機能・変更は当然カバーされていません
