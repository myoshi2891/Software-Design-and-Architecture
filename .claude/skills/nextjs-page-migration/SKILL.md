...
<ai_agent_directive>
**AI エージェントへの厳格な指示**: 以下のプロセスは**ゲート条件（Gate Condition）**です。タスクの報告を行う前に、ステップバイステップでビルド、リント、テストを実行してください。ただし、ルール違反（手順のスキップや一括コミットなど）を検知した場合は、独断でリカバリ操作を行わず、直ちにユーザーに報告し、承認を得てからリカバリ手順を実行してください。
</ai_agent_directive>
