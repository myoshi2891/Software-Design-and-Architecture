...
前提知識として、SQLの基本的なテーブル操作（SELECT・JOINなど）を知っていればで十分です。特定のNoSQL製品の操作経験は不要です。
...
2. **クラスタの襲来（Attack of the Clusters）**
   GoogleやAmazonのような大規模サービスは、1台の巨大なサーバーではなく、安価なコモディティサーバーを大量に束ねたクラスタでデータを扱う必要に迫られました。しかしRDBはそもそも「クラスタで動かす」ことを前提に設計されていません。
...
いずれの方法でも、集約をまたぐ「結合（JOIN）」に相当する処理は、RDBのように宣言的にはできず、アプリケーションコード側で複数回のアクセスに分解して実装することになります。
...
グラフDBのクエリは「隣接するノードをたどる」処理に最適化されており、「友人の友人で、かつ同じ商品を買った人」のような多段階の関係をたどる問い合わせが、RDBの多段JOINよりもはるかに高速に処理できます。本書はNeo4jを代表例として扱っています。
...
```mermaid
flowchart TD
    CAP["CAP定理<br/>ネットワーク分断が起きたら<br/>CとAのどちらかを選ぶ"]
    CAP --> C["一貫性 Consistency<br/>全ノードが同じ値を返す"]
    CAP --> A["可用性 Availability<br/>常に応答を返す"]
    CAP --> P["分断耐性 Partition Tolerance<br/>ネットワーク分断でも動き続ける"]
    P -->|"分断発生時に選ぶ"| Choice{"CかAか"}
    Choice -->|"Cを優先"| CP["CP型<br/>例: 一部のRDB系分散DB"]
    Choice -->|"Aを優先"| AP["AP型<br/>例: Riak, Cassandra系"]
```
...
分散システムでは、レプリカの台数を N、書き込み成功に必要な台数を W、読み取りに問い合わせる台数を R とすると、`W + R > N` を満たすように設定すると、読み取りと書き込みの間で必ず重なりが生じ、最新の書き込みを読み取れることが保証しやすくなります。このW・R・Nの組み合わせを調整することで、一貫性と可用性のバランスを細かく設計できます。
...
- **一貫性**：ピアツーピア型で分散する実装が多く、書き込み競合の解決にバージョンスタンプ（Step5）を用いる
- **トランザクション**：1つのキーに対する操作はアトミックだが、複数キーにまたがるトランザクションは基本的にサポートされない
...
- 複数のドキュメントをまたぐ複雑なトランザクションが必要な場合
...
Googleの Bigtable 論文に着想を得たモデルで、行キーに対して、可変長の「列（カラム）」の集合をまとめて保存します。列はあらかじめ固定されておらず、行ごとに異なる列を持つことができます。列同士をグループ化したものを「カラムファミリー」と呼びます。本書ではCassandraを主な例に、HBaseにも触れています。
...
- **関係のたどりやすさ**：JOINを何段も重ねるRDBのクエリに比べ、隣接ノードをたどる操作が定数時間に近い速度で実行できる（インデックスフリー隣接）
...
| ドキュメント | MongoDB, CouchDB, RavenDB | 柔軟なスキーマ、フィールド検索 | ドキュメントをまたぐトランザクション |
...
### 本書の看板例だったRiakは、事実上開発が止まっている

本書はキーバリューストアの代表例としてRiakを繰り返し取り上げていますが、Riakの開発元Basho Technologiesは2017年に資金繰りに行き詰まって経営破綻し、資産はBet365に買収されました。GitHub上のリポジトリは有志によって細々とメンテナンスされてきたものの、2023年頃を最後に更新が止まっており、データベースカタログサイト「Database of Databases」では「abandoned（放棄された）」と分類されています。本書でRiakの動作例を読むときは、「今から新規に採用する製品」としてではなく、「ピアツーピア型レプリケーションを説明するための教材」として読むのが実用的です。
...
- **DB-Enginesランキング**では、2026年9月時点でもMongoDBが文書データベース／NoSQL全体で高い人気を維持し、Redisはキーバリュー・インメモリ分野で圧倒的なシェアを持ち、Neo4jはグラフDB分野を依然としてリードしています
...
- **本書が扱っていない領域**：2012年刊行のため、当然ながらベクトルデータベース（Qdrant、Milvusなど）のようなAI・機械学習ワークロード向けの新しいカテゴリは扱われていません。これらは「集約指向」でも「グラフ」でもない、第5のデータモデルとして2020年代に急速に発展した領域です
...
| 集約（Aggregate） | ひとまとまりとして扱うデータの集合。ACID操作やクラスタ分散の単位になる |
