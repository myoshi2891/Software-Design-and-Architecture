...
なお、Read Repair（読み取り時にレプリカ間の差分を検出して遅れたレプリカへ書き戻す仕組み）や Active Anti-Entropy（バックグラウンドで Merkle ツリーなどを用いて差分を継続的に修復する仕組み）は、これらとは役割が異なります。これらはレプリカ集合全体を収束させて不整合の残存期間を短くするための仕組みであって、ある読み取りが最新値を返すための必須条件ではありません。`W + R > N` が成立していれば、修復が未実施でも読み取り時点のレプリカ集合に最新値は含まれます。
