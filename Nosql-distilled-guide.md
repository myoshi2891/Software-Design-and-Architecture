...
- バージョン（ベクタークロックやタイムスタンプ）を比較して、取得した複数の値から最新のものを選び出せること
- 同時更新による衝突をどう解決するかが定義されていること
...
- **関係のたどりやすさ**：各ノードが隣接ノードへの参照を直接保持するため（インデックスフリー隣接）、1ホップ分のたどりにグローバルなインデックス探索が不要になる。ただし可変長パスの探索では、ホップ数と分岐数に応じて生成されるパスが爆発的に増えて大幅に遅くなることがあり、「常に高速」ではない
