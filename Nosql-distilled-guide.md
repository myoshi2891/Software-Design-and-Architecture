...
本書はわずか192ページ（実質152ページ）の薄い本ですが、その分「特定の製品の使い方」ではなく「NoSQLというジャンル全体を貫く概念」を凝縮して説明することに徹しています。だからこそ、2012年の初版刊行から10年以上経った今でも、多くの現場で「NoSQLを学ぶなら最初の1冊」として紹介され続けています。
...
flowchart LR
    ...
    E --> F["当時主流の単一ノード中心の<br/>RDB製品はクラスタ向けに<br/>設計されていない"]
    B --> G["NoSQLの誕生"]
    F --> G
...
- 当時広く使われていた従来の単一ノード中心のRDB製品は、クラスタで効率よく動くようには設計されていなかった
...
キーバリュー・ドキュメント・カラムファミリーの3つに共通するのが「**集約（Aggregate）**」という単位です。集約とは、ドメイン駆動設計（DDD）由来の概念で、「ひとまとまりとして扱いたいデータの集合」を指します。たとえば注文（Order）であれば、注文明細・配送先住所に加え、注文時点の顧客情報（氏名など）のスナップショットを1つの集約としてまとめて保存し、1回の操作で読み書きします。顧客そのものは別の顧客（Customer）集約として管理し、注文集約からは顧客IDで参照します。
...
flowchart TD
    ...
    AGG --> Note["クラスタ分散の境界になりうる<br/>(原子的な更新の境界になるのは<br/>データストアの原子性単位が<br/>集約全体を覆う場合のみ)"]
```
...
| クラスタ分散のしやすさ | 集約単位で分散しやすい（原子性の単位とは独立に、集約を分散の境界にできる） | 関係をまたぐため分散は難しくなりがち |
| 原子的な更新 | データストアの原子性単位が集約全体を覆う場合に限り、集約内の更新を原子的に行える | 製品ごとのトランザクション機能に依存 |
| 向いている例 | ショッピングカート、ユーザープロファイル | ソーシャルグラフ、レコメンド、不正検知 |

### Key Points（第2章のまとめ）

- 集約とは、ひとまとまりとして扱うデータの集合である。集約自体が原子性を保証するわけではなく、データストアの原子性の単位が集約全体を覆う場合に限って、ACID操作の境界になる
...
- 集約をまたぐ関係は、キー参照または埋め込みのどちらかで表現する。集約をまたぐ結合機能の有無は製品依存であり（例: MongoDBの`$lookup`）、サポートされる結合の種類、シャーディングなど構成上の制限、性能特性は製品ごとに異なる
...
レプリケーションされた環境では、更新がすべてのレプリカに伝播するまでに時間差が生じます。この間に読み取りを行うと、ノードによって異なる値が返る可能性があります。これが**結果整合性（Eventual Consistency）**――「対象データへの新たな更新が止まれば、いずれは全ノードが同じ値に収束するが、収束する瞬間は保証されない」という考え方です。
...
なお、Read Repair（読み取り時にレプリカ間の差分を検出して遅れたレプリカへ書き戻す仕組み）や Active Anti-Entropy（バックグラウンドで Merkle ツリーなどを用いて差分を継続的に修復する仕組み）は、これらとは役割が異なります。これらはレプリカ集合全体を収束させて不整合の残存期間を短くするための仕組みであって、ある読み取りが最新値を返すための必須条件ではありません。`W + R > N` だけで最新値が保証されるわけではありません。読み書きが同じレプリカ集合を対象とし（スロッピー・クォーラムを使わない）、上記のバージョン比較と衝突解決が正しく最新値を特定できるという前提のもとでは、読み取りクォーラムと書き込みクォーラムが重なるため、修復が未実施でも読み取り時点で問い合わせたレプリカに最新値が含まれ、それを選び出せます。
...
- 結果整合性は、新たな更新が止まればいずれ全ノードが収束することだけを保証し、収束までの時間は保証しない
...
大規模なMap-Reduceでは、Reduceの前段に「Combiner」を挟み、各ノード内であらかじめ部分集計をしてからシャッフルすることで、ノード間の通信量を減らす最適化が一般的です。ただしCombinerは省略可能な最適化であり、安全に使えるのはReduce処理が結合的かつ可換な場合に限られます。実装によってCombinerは0回・1回・複数回のいずれでも呼ばれうるため、呼び出し回数に結果が依存してはいけません。たとえば合計は部分和をさらに合計しても正しい結果になりますが、平均は「平均の平均」を繰り返すと件数の重みが失われて誤った値になるため、Combinerでは（合計, 件数）の組を部分集計し、最後のReduceで割り算する必要があります。
...
- キー以外の条件で値を検索したい場合
- 複数キーにまたがる操作をアトミックに行いたいが、選択した製品・構成がそれに対応していない、または対応していても性能要件を満たさない場合
...
- **Cassandra**：現行のCQLでは、列はテーブルスキーマとして定義します。値を設定しない列があっても構わず、その分の記憶域も消費しない（疎である）一方で、Bigtableのように行ごとへ任意の列を後から足せるわけではありません。動的な属性を表現したい場合、件数が少なく上限が見込める属性なら `map` などのコレクション型が適します（コレクションは読み出し時に全体が取得されるため、肥大化させる用途には向きません）。一方、件数が増え続けるデータや、要素ごとに検索・範囲取得したいデータは、クラスタリングカラムで識別・整列される別の行として表現します。両者は「列を増やすための等価な代替手段」ではなく、データの量と検索要件によって使い分けるものです。
...
flowchart TD
    ...
    Doc --> Docng["苦手になりうる: 複数集約を<br/>横断するトランザクション<br/>（製品・構成・性能要件による）"]
...
|---|---|---|---|
| キーバリュー | Riak, Redis, Memcached, Amazon DynamoDB | 高速な単純取得、キャッシュ | 値の中身での検索、複数キーの整合性（製品・配置構成が複数キーのアトミック操作に対応していない場合、または対応していても性能要件に合わない場合。例: Redisは単一ノードならMULTI/EXECで扱えるが、クラスタ構成では同一ハッシュスロットのキーに限られる） |
...
timeline
    ...
    2017 : Kleppmann「Designing Data-Intensive Applications」初版刊行
    2020年代前半 : クラウドマネージド型が主流に（DynamoDB, Cosmos DB等）
    2026 : Kleppmann・Riccomini「Designing Data-Intensive Applications」第2版刊行
...
- **現在のRiak（OpenRiak コミュニティ）**: その後、有志による `OpenRiak` 組織へ開発が引き継がれています。Riak KV は2026年1月に3.4.0、2026年6月に3.4.1がリリースされており、本ガイド執筆時点（2026年9月）でも各コンポーネントのリポジトリが更新され続けています（[OpenRiak公式リリース一覧](https://github.com/OpenRiak/riak/releases)、参考文献19）。
...
本書の後継として2017年に初版が刊行されたMartin Kleppmannの『Designing Data-Intensive Applications』（通称DDIA）は、2026年3月にChris Riccominiを共著者に迎えた第2版（O'Reilly Media）が刊行されています。DDIAは、NoSQL Distilledが切り開いた「データストアを俯瞰して比較する」というアプローチを引き継ぎつつ、ストレージエンジンの内部構造、レプリケーションの詳細、トランザクションの分離レベル、ストリーム処理までを、より深く現代的な事例とともに扱っています。NoSQL Distilledを読み終えて「もっと深く知りたい」と思った読者の多くが、次に手に取る本として挙げています。これから読むなら、最新の第2版を選ぶのがおすすめです。
...
2. **代表例の陳腐化**：上述の通り、Basho時代のRiak（旧来の `basho/riak` 系統）の開発停止のように、10年以上前の「今のイチオシ」は現在では通用しないことがあります。個別の製品名よりも、その背後にある「データモデル」の考え方を吸収する読み方が長持ちします
3. **CAP定理の単純化**：本書のCAP定理の説明はわかりやすい反面、提唱者自身や後続の研究者による精緻化を踏まえると、「2つしか選べない」という理解だけで止まらないほうがよいでしょう
4. **薄い本ゆえの割り切り**：著者ら自身が「飛行機の中で読み切れる分量」を意図したと述べている通り、本書は意図的に各トピックを深掘りしすぎない設計になっています。実装の詳細（各製品のドライバの使い方、具体的なクエリ構文など）は別途、各製品の公式ドキュメントやKleppmann・RiccominiのDDIA第2版のような専門書で補う必要があります
...
| 集約（Aggregate） | ひとまとまりとして扱うデータの集合。クラスタ上ではまとめて同じノードに配置される分散の単位となる。また、データストアの原子性単位が集約全体を覆う場合に限り、ACID操作の単位にもなりうる |
...
| 結果整合性 | 対象データへの新たな更新が止まれば、いずれすべてのレプリカが同じ値に収束するという一貫性モデル。収束までの時間は保証されない |
...
- [ ] 集約（Aggregate）という考え方と、それがクラスタ分散の単位になる理由、およびデータストアの原子性単位が集約全体を覆う場合に限ってACID操作の境界になる理由を説明できる
...
一方で、具体的な製品事例（特にRiak）や、CAP定理の単純化された説明は、2026年時点の知見を踏まえて補いながら読む必要があります。本書で概念の地図を手に入れたら、次はMartin KleppmannとChris RiccominiのDesigning Data-Intensive Applications（第2版, 2026）のような、より現代的で詳細な一冊に進むのがおすすめの学習パスです。
...
10. Martin Kleppmann, Chris Riccomini: Designing Data-Intensive Applications, 2nd Edition（O'Reilly Media, 2026）公式サイト
...
19. OpenRiak: Riak 公式リリース一覧（3.4.0・3.4.1 など OpenRiak 体制でのリリース）
    https://github.com/OpenRiak/riak/releases
