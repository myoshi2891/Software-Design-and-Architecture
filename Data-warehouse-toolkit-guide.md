...
| データ構造 | 正規化（第3正規形が基本） | 非正規化されたディメンショナルモデル（スタースキーマ） |
...
原著第3章では「ファクトレス・ファクトテーブル」も紹介されています。数値のファクトを一切持たず、複数のディメンションキーの組み合わせだけで「何かが発生した」という事実、または「あるディメインの組み合わせがカバー範囲として存在する」という事実を記録するテーブルです（例: 学生の出席記録、店舗と商品の取扱可否の組み合わせ）。
...
| Type 4（履歴テーブル分離） | 現在値は主ディメンションに、変更履歴は別のヒストリーテーブルに保持する | 主ディメンションを小さく保てる | 履歴を見るには別テーブルとのJOINが必要 |
| Type 5 | ミニディメンション + Type 1参照の組み合わせ | 頻繁に変わる属性群を切り離して管理できる | 設計・運用がやや複雑 |
| Type 6（1+2+3のハイブリッド） | Type 2で履歴行を追加しつつ、Type 1的な「現在値」列とType 3的な「直前値」列も同時に持つ | 履歴分析と最新値分析を1つのテーブルで両立できる | 設計・ETLロジックが最も複雑 |
| Type 7（デュアルキー） | Type 6と同等の機能を、物理的な列上書きではなく自然キー・サロゲートキーの2種類のキーを使い分けて実現する | Type 6より実装がシンプルになるケースがある | やはり相応の設計スキルが必要 |
...
| ファクトテーブル | 数値の測定値（ファクト）とディメンションへの外部キーを持つテーブル |
